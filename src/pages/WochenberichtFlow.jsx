  ...
  const steps = [
    ...
    { t: 'Stunden · Kunden · Payback', node: (
      <>
        ...
        <Deriv label="Differenz (Std.)" v={b.stundenDiff} on={(x) => setDeriv('stundenDiff', x)} reset={() => resetDeriv('stundenDiff')} auto={!manual.has('stundenDiff')} />
  ...
  return (
    <>
      ...
      <div className="page">
        ...
        <h2 style={{ fontSize: 18, marginBottom: 12 }}>{steps[step].t}</h2>
...
function Feld({ label, v, on }) {
  return (
    <div className="field">
      <label>{label}</label>
      <input inputMode="decimal" value={dv(v)} onChange={(e) => on(e.target.value)} />
    </div>
  )
}
